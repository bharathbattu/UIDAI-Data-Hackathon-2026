...
Upload ZIP datasets to Google Drive

Run all cells in Notebook 01 (~5 minutes)

Run all cells in Notebook 02 (~30 minutes)
...
Key Visualizations
Geographic Concentration - Top 15 states by enrolment volume (horizontal bar chart)

Age Distribution - Child, teen, adult breakdown across top 20 states (stacked bar chart)

Update Behavior - State-wise demographic vs biometric update patterns (scatter plot)

Monthly Trends - Time-series analysis with peak month highlighting (line chart with trend)

State Growth Patterns - Month-on-month trends for top 5 states (multi-line chart)
...
Extract to data/raw/ folder (for local execution)

For Google Colab: Upload to Google Drive
...
UIDAI Data Portal: event.data.gov.in

OGD Platform India: data.gov.in

📋 Submission Details
Final Submission: View PDF Report
